# Variable String will always be equal to a string contained in array1
VariableString = None
array0 = {
...
array3 = [x.name for x in DiscordCTXObject.author.roles]
if any(x in array2 for x in list(array0)):
  array4 = [x for x in array2 if x in array1]
  array5 = [x for x in array3 if x in array1]
  if array1.index(array0[array4[-1]][0]) <= array1.index(VariableString) and array1.index(array0[array4[-1]][0]) <= array1.index(array5[-1]):
    #...
    return
